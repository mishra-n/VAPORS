...
def fit(spectrum, model, mask_dict, maxiter=1000, algorithm='LevMarLSQFitter'):
    """
    Fit a custom model to a spectrum, considering specific spectral lines and redshifts.
    ...
    # Initialize the fitting algorithm with uncertainty calculation
    ve_fit = None
    if algorithm == 'TRFLSQFitter':
        ve_fit = fitting.TRFLSQFitter(calc_uncertainties=True)
    elif algorithm == 'LevMarLSQFitter':
        ve_fit = fitting.LevMarLSQFitter(calc_uncertainties=True)    
    # Initialize the mask to None
    mask = None
    ...
    for entry in mask_dict['redshifts']:
        ...
        for line in entry['lines']:
            transition = SEARCH_LINES[SEARCH_LINES['tempname'] == line]['wave'][0] #* u.Angstrom
            vranges = entry['vrange'].get(line, None)
            # reuse helper to generate mask for this line's vranges
            mask_temp = mask_for_line(spectrum.spectral_axis, transition, z, vranges)
            # print the number of true values in the mask
            # print(f"Mask for {line} at z={z}: {np.sum(mask_temp)} true values")
            # Combine the masks for different lines
            if mask is None:
                mask = mask_temp
            else:
