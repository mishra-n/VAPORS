...
def genSummaryTable(spectrum, chain, fit, upper_limits, non_mcmc_upper_limits):
    ...
    colnames = ['ion', 'median_N', 'N_left_error', 'N_right_error', 'upper_limit', 'non_MCMC_upper_limit', '2-sigma_UL_N', 'v', 'v_righterr' , 'v_lefterr', 'b_other', 'b_righterr', 'b_lefterr','T', 'T_lefterr', 'T_righterr', 'mass', 'z', 'absorber_number']
    dtype = [   'U10', 'float64',  'float64',      'float64',       'bool',        'bool',                'float64',  'float64', 'float64',    'float64',   'float64', 'float64',    'float64', 'float64','float64','float64','float64','float64', 'int']
    ...
    for i, param in enumerate(fit.cov_matrix.param_names):
        ...
        new_row = (ion_name, N_median, N_left_error, N_right_error, False, False, np.nan, v_median,v_left_error,v_right_error, b_median,b_left_error,b_right_error, T_median,T_left_error,T_right_error, mass_value, z, 0)
    ...
    for i, absorber_num in enumerate(np.unique(table['absorber_number'])):
        for j, line_name in enumerate(upper_limits):
            ...
            if ((table['ion'] == name) * (table['absorber_number'] == absorber_num)).sum() == 0:
                ...
                new_row = (name, np.nan, np.nan, np.nan, True, False, np.log10(2*NErr * u.cm**2), v_median,v_left_error,v_right_error, b_median,b_left_error,b_right_error, T_median,T_left_error,T_right_error, mass, z, absorber_num)
                table.add_row(new_row)

            else:
                index = np.where((table['ion'] == name) * (table['absorber_number'] == absorber_num))
                compare = table[table['ion'] == name]['2-sigma_UL_N'].mean()
                if 2*NErr.value < compare:
                    new_row = (name, np.nan, np.nan, np.nan, True, False, np.log10(2*NErr * u.cm**2), v_median,v_left_error,v_right_error, b_median,b_left_error,b_right_error, T_median,T_left_error,T_right_error, mass, z, absorber_num)
                    table[index] = (new_row)
        ...
        for j, line_name in enumerate(non_mcmc_upper_limits):
            ...
            if ((table['ion'] == name) * (table['absorber_number'] == absorber_num)).sum() == 0:
                ...
                new_row = (name, np.nan, np.nan, np.nan, False, True, np.log10(2*NErr * u.cm**2), v_median,v_left_error,v_right_error, b_median,b_left_error,b_right_error, T_median,T_left_error,T_right_error, mass, z, absorber_num)
                table.add_row(new_row)
