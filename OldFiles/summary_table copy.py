...
def genSummaryTable(spectrum, chain, fit, upper_limits, non_mcmc_upper_limits):
    ...
    colnames = ['ion', 'median_N', 'N_left_error', 'N_right_error', 'upper_limit', 'non_MCMC_upper_limit', '2-sigma_UL_N', 'v', 'b_other', 'T', 'mass', 'z', 'absorber_number']
    dtype = [   'U10', 'float64',  'float64',      'float64',       'bool',        'bool',        'float64',     'float64', 'float64', 'float64', 'float64','float64', 'int']
    ...
    for i, param in enumerate(fit.cov_matrix.param_names):
        ...
        median = np.median(param_posterior)
        left_error = median - np.percentile(param_posterior, 16)
        right_error = np.percentile(param_posterior, 84) - median

        new_row = (ion_name, median, left_error, right_error, False, False, np.nan, v_val, b_other_val, T_val, mass_value, z, 0)
    ...
    for i, absorber_num in enumerate(np.unique(table['absorber_number'])):
        for j, line_name in enumerate(upper_limits):
            b_other = table[table['absorber_number'] == absorber_num]['b_other'].mean()
            T = table[table['absorber_number'] == absorber_num]['T'].mean()
            v_val = table[table['absorber_number'] == absorber_num]['v'].mean()
            ...
            b_tot = np.sqrt(b_other**2 + b_therm**2)
            ...
            if ((table['ion'] == name) * (table['absorber_number'] == absorber_num)).sum() == 0:
                ...
                new_row = (name, np.nan, np.nan, np.nan, True, False, np.log10(2*NErr * u.cm**2), v_val, b_other, T, mass, z, absorber_num)
                table.add_row(new_row)
        ...
        for j, line_name in enumerate(non_mcmc_upper_limits):
            ...
            if ((table['ion'] == name) * (table['absorber_number'] == absorber_num)).sum() == 0:
                ...
                new_row = (name, np.nan, np.nan, np.nan, False, True, np.log10(2*NErr * u.cm**2), v_val, b_other, T, mass, z, absorber_num)
                table.add_row(new_row)
